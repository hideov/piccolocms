...
class FlintstoneDB
{
    ...
    /**
     * Swap two entries in the database
     *
     * @param string $key1, $key2 the two keys to swap
     *
     * @throws FlintstoneException when one of the keys is invalid
     *
    ...
    public function swap($key1, $key2)
    {
        ...
        foreach ($filepointer as $line) {
            ...
            if ($pieces[0] == $key1) {
                $line1 = $line;
            } elseif ($pieces[0] == $key2) {
            $line2 = $line;
            }
        }
        $this->closeFile($filepointer);

        // swap the lines in the temp file
